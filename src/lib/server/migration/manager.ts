...
export async function updateSource(
	...
	if (updates.label !== undefined) set.label = updates.label.trim();
	if (updates.config !== undefined) set.config = updates.config;
	const res = await collections.migrationSources.findOneAndUpdate(
		{ _id },
		{ $set: set },
...
async function runJob(
	...
	const skippedPhases: Record<string, string> = {};
	const ctx: MigrationConnectorContext = {
		...
		reportSkip: (phase, reason) => {
			skippedPhases[phase] = reason;
			void collections.migrationJobs.updateOne(
				{ _id: jobId },
				{ $set: { skippedPhases, updatedAt: new Date() } }
			);
		}
	};
	...
	await collections.migrationJobs.updateOne(
		{ _id: jobId },
		{ $set: { status: 'done', finishedAt, counts, skippedPhases, updatedAt: finishedAt } }
	);
}
...
export async function promoteStaged(
	...
	// Re-promote is allowed: each promote creates a new be-BOP entity
	// (or re-applies for settings). Previous entity is not auto-deleted —
	// admin cleans up if needed. promotedAsId on the staged object is
	// overwritten with the latest promotion's id.
