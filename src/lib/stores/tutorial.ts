...
import type { SerializedTutorial, TutorialStep } from '$lib/types/Tutorial';
...
export interface TutorialState {
	...
	currentTutorial: SerializedTutorial | null;
	currentProgress: Partial<TutorialProgress> | null;
}
...
function createTutorialStore() {
	...
	return {
		 ...
		 */
		initialize: (tutorial?: SerializedTutorial | null): number => {
			const persisted = getPersistedState();
			if (
				persisted &&
				persisted.isActive &&
				(persisted.status === 'waiting_for_navigation' || persisted.status === 'running')
			) {
		 ...
		 */
		startTutorial: (tutorial: SerializedTutorial, progress?: Partial<TutorialProgress>) => {
