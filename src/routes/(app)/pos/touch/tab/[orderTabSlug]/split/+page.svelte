<script lang="ts">
	...
	// Calculate original tab total in main currency (CHF) using perItem prices + VAT
	$: originalTabTotalWithVat = originalQuantitiesPriceInfo.perItem.reduce(
		(sum, item, i) => sum + item.amount * (1 + (originalQuantitiesPriceInfo.vatRates[i] ?? 0) / 100),
		0
	);
					...
					<div class="flex flex-col h-full gap-4">
						...
						>
							...
							<input
								type="hidden"
								name="customAmount"
								value={tabItemsPriceInfo.partialPriceWithVat}
							/>
							...
							<div class="grid grid-cols-3 gap-3">
								{#each Array.from({ length: POS_SPLIT_SHARES_MAX_NUMS }, (_, i) => i + 1) as num}
									<form method="POST" action="?/checkoutTabPartial" use:enhance>
										...
										<button
											type="submit"
											class="bg-yellow-800 text-white font-bold text-4xl p-4 rounded w-full"
										>
											{num}
										</button>
									</form>
						...
						{#if data.sharesOrder}
							<div class="bg-gray-100 p-6 rounded-lg space-y-3">
								...
								{:else}
									{@const remaining =
										data.sharesOrder.remainingToPay ??
										originalTabTotalWithVat - data.sharesOrder.totalAlreadyPaid}
