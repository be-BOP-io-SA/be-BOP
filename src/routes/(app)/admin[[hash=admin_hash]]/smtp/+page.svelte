...
<h1 class="text-3xl">{t('admin.smtp.title')}</h1>
		...
		<div class="flex items-start">
			...
			<div class="ml-3">
				<h3 class="text-sm font-medium text-yellow-800">
					{t('admin.smtp.envVarsWarningTitle')}
				</h3>
				<div class="mt-2 text-sm text-yellow-700">
					<p>
						{t('admin.smtp.envVarsWarningText')}
					</p>
...
<form class="contents" method="post" action="?/save">
	...
	>
		<input
			...
			type="checkbox"
		/>
		{t('admin.smtp.useFakeSmtp')}
		<span class="text-sm text-gray-500 ml-2">{t('admin.smtp.useFakeSmtpHint')}</span>
	</label>
	<div class="grid grid-cols-1 md:grid-cols-2 gap-4 {disableForm ? 'opacity-50' : ''}">
		...
		<label class="form-label">
			...
			<input
				...
				placeholder={data.smtp.passwordIsSet
					? t('admin.smtp.passwordSetPlaceholder')
					: t('admin.smtp.passwordPlaceholder')}
				type="password"
				value=""
			/>
		</label>
		...
		<label class="form-label">
			...
			/>
			<span class="text-sm text-gray-500">{t('admin.smtp.fromEmailHint')}</span>
		</label>
	</div>

	<div class="flex justify-between mt-6">
		<button class="btn btn-black" type="submit" disabled={disableSubmitButton}
			>{t('admin.smtp.save')}</button
		>
...
<form
	...
	id="delete-form"
	use:enhance={(e) => {
		if (!confirm(t('admin.smtp.removeConfirm'))) {
			e.cancel();
		}
	}}
></form>
...
<div class="mt-8 p-4 bg-blue-50 rounded-lg">
	<h3 class="text-lg font-semibold mb-2">{t('admin.smtp.notesTitle')}</h3>
	<ul class="text-sm space-y-1 text-gray-700">
		<li>• {t('admin.smtp.notePort587')}</li>
		<li>• {t('admin.smtp.notePort465')}</li>
		<li>• {t('admin.smtp.notePort25')}</li>
		<li>• {t('admin.smtp.noteFakeMode')}</li>
		<li>• {t('admin.smtp.noteOverrides')}</li>
	</ul>
</div>
