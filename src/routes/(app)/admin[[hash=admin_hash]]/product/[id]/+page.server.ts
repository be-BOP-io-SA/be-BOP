...
export const actions: Actions = {
	update: async ({ request, params }) => {
		...
		const stockReferenceProductId = formData.get('stockReferenceProductId');
		if (stockReferenceProductId && stockReferenceProductId !== '') {
			const validation = await validateStockReference(params.id, stockReferenceProductId as string);

			if (!validation.valid) {
				console.warn(
					`Stock reference validation failed: ` +
						`product=${params.id}, ` +
						`reference=${stockReferenceProductId}, ` +
			...
			const res = await collections.products.updateOne(
				{ _id: params.id },
				{
					$set: {
						...
						...(stockReferenceProductId &&
							stockReferenceProductId !== '' && {
								stockReference: {
									productId: stockReferenceProductId as string
								}
							})
					},
					$unset: {
						...
						...(!stockReferenceProductId && { stockReference: '' }),
