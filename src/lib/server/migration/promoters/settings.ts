 ...
 * within the same migration job. Returns null if not found.
 *
 * Scoping to the job's `jobId` is important: across multiple migrations the
 * same `wp_media_<id>` sourceId may exist on unrelated staged images, and
 * picking the wrong one would attach an unrelated Picture to settings.
 */
async function findPromotedImage(
	jobId: import('mongodb').ObjectId,
	wpMediaId: number
): Promise<string | null> {
...
export const settingsPromoter: MigrationPromoter = {
	...
	async proposedChanges(staged) {
		...
		if (typeof n.siteLogoMediaId === 'number') {
			const promotedAsId = await findPromotedImage(staged.jobId, n.siteLogoMediaId);
			changes.push({
				key: 'logo.pictureId',
				label: 'Site logo',
		...
		if (typeof n.siteIconMediaId === 'number') {
			const promotedAsId = await findPromotedImage(staged.jobId, n.siteIconMediaId);
			changes.push({
	...
	async promote(staged, options) {
		...
		if (accepted.has('logo.pictureId') && typeof n.siteLogoMediaId === 'number') {
			const pictureId = await findPromotedImage(staged.jobId, n.siteLogoMediaId);
			if (pictureId) {
		...
		if (accepted.has('faviconPictureId') && typeof n.siteIconMediaId === 'number') {
			const pictureId = await findPromotedImage(staged.jobId, n.siteIconMediaId);
			if (pictureId) {
