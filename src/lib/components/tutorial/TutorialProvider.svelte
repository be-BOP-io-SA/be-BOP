<script lang="ts">
	...
	export let tutorial: SerializedTutorial | null = null;
	export let progress: { tutorialId: string; tutorialVersion: number } | null = null;
	...
	beforeNavigate(() => {
		console.log('[Tutorial] beforeNavigate', {
			...
			currentStep: $tutorialStore.currentStepIndex
		});
		...
		try {
			...
			if (stepDef.skipCondition) {
				...
				if (shouldSkip) {
					...
					const skipText =
						skipReason === 'already_configured'
							? t('tutorial.common.alreadyConfigured') ||
							  'This is already configured. Moving to the next step...'
							: t('tutorial.common.stepNotNeeded') ||
							  'This step is not needed. Moving to the next step...';
			...
			if (!element) {
				console.error(
					'[Tutorial] Cannot show step',
					stepDef.id,
					', element not found:',
					stepDef.attachTo.element
				);
				return;
			}
			...
			if (isLastStep) {
				...
				buttons.push({
					...
					classes: hasRequiredAction
						? 'shepherd-button-primary shepherd-button-disabled'
			...
			const stepCounter =
				t('tutorial.common.stepCounter', {
					current: state.currentStepIndex + 1,
					total: tutorial.steps.length
				}) || `Step ${state.currentStepIndex + 1} of ${tutorial.steps.length}`;
	...
	function setupRequiredActionMonitoring(stepDef: SerializedTutorial['steps'][0]) {
		if (!stepDef.requiredAction) {
			return;
		}
		...
		const enableNextButton = () => {
			...
			const nextBtn =
				buttons && buttons.length > 0 ? (buttons[buttons.length - 1] as HTMLButtonElement) : null;
			console.log(
				'[Tutorial] enableNextButton - footer:',
				!!footer,
				'buttons:',
				buttons?.length,
				'nextBtn:',
		...
		if (type === 'input' && validation === 'non-empty') {
			...
			console.log(
				'[Tutorial] Setting up input monitoring for',
				selector,
				'current value:',
				input.value
			);
			...
			if (targetElement) {
				...
				const handleClick = () => {
					...
					sessionStorage.setItem(
						'bebop-tutorial-action-completed',
						JSON.stringify({
							stepId: stepDef.id,
