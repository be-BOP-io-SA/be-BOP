					...
					case 'taler':
						try {
							...
							if (talerOrder === 'not_found') {
								...
								if (!talerOrder.deposit_total || !talerOrder.deposit_total.includes(':')) {
									throw new Error(`Invalid deposit_total format: ${talerOrder.deposit_total}`);
								}
								const amountStr = talerOrder.deposit_total.split(':')[1];
