...
export interface MigrationConnectorContext {
	...
	/**
	 * Declare that a phase was skipped (e.g. blocked by insufficient
	 * permissions on the source). Surfaced in the migration job detail UI
	 * so admins know what didn't make it without grepping server logs.
	 */
	reportSkip(phase: string, reason: string): void;
}
