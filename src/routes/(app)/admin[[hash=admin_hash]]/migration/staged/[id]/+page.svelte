<script lang="ts">
	...
	$: hasProposedChanges = !!data.proposedChanges && data.proposedChanges.length > 0;
	$: alreadyPromoted = data.staged.status === 'promoted';
...
<section class="mt-6">
		...
		{:else if !hasProposedChanges}
			<form method="post" use:enhance class="inline">
				<button class="btn btn-black" formaction="?/promote">
					{alreadyPromoted ? `Re-${data.promoter.actionLabel.toLowerCase()}` : data.promoter.actionLabel}
				</button>
			</form>
		{/if}
	...
	{#if alreadyPromoted}
		<p class="text-xs text-gray-600 mt-3">
			Already promoted as <span class="font-mono">{data.staged.promotedAsId}</span>. Promoting
			again creates another be-BOP entity (the previous one stays — delete it manually if you
			want to replace it).
		</p>
	{/if}
</section>
...
{#if hasProposedChanges && data.proposedChanges && data.promoter}
	<section class="mt-6">
		...
		<p class="text-sm text-gray-600 mb-3">
			...
			{#if alreadyPromoted}
				This object was already promoted — applying again will overwrite the matching values.
			{/if}
		</p>
		<form method="post" action="?/promote" use:enhance class="flex flex-col gap-3">
			<AcceptIgnoreList items={data.proposedChanges} />
			<button class="btn btn-black self-start">
				{alreadyPromoted ? `Re-${data.promoter.actionLabel.toLowerCase()}` : data.promoter.actionLabel}
			</button>
		</form>
	</section>
