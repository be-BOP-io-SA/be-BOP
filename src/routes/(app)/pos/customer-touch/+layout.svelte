...
<main class="fixed top-0 bottom-0 right-0 left-0 bg-white px-4 overflow-scroll">
	<div class="flex items-center justify-between w-full">
		...
		<div class="flex flex-col items-center">
			{#if data.logoPicture}
				<Picture class="dark:hidden w-24 h-16" picture={data.logoPicture} />
				<Picture
					class="hidden dark:inline  w-24 h-16"
					picture={data.logoPictureDark || data.logoPicture}
				/>
			{:else}
				<img class="hidden dark:inline w-24 h-16" src={DEFAULT_LOGO} alt="" />
				<img class="dark:hidden w-24 h-16" src={DEFAULT_LOGO_DARK} alt="" />
			{/if}
