...
<form method="post" action="?/update" use:enhance class="flex flex-col gap-4">
	{#if data.digitalFile.productId}
		<a href="{data.adminPrefix}/product/{data.digitalFile.productId}" class="underline text-center">
			Back to product
		</a>
	{:else}
		<a href="{data.adminPrefix}/digital-file" class="underline text-center"> Back to files </a>
	{/if}

	<input type="text" name="name" class="form-input" value={data.digitalFile.name} />
	<a href={data.downloadLink} target="_blank" class="underline text-center">Download file</a>
